...
const roleRouteAccess = {
  ADMIN: ["/admin", "/api/users", "/api/plans", "/api/households", "/api/members", "/api/cards", "/api/audit-logs"],
...
export default async function middleware(req: NextRequestWithAuth) {
  ...
  if (["/auth/signin", "/auth/error"].some(p => path.startsWith(p))) {
  ...
  if (path.startsWith("/api/")) {
    ...
    const hasAccess = roleRouteAccess[userRole].some(route => 
  ...
  if (
    ...
    (path.startsWith("/agent") && userRole !== "ADMIN" && userRole !== "OFFICE_AGENT") ||
    (path.startsWith("/hospital") && !["ADMIN", "OFFICE_AGENT", "HOSPITAL_USER"].includes(userRole))
  ) {
